...
#define SITRONIX_RESET_GPIO1	EXYNOS4_GPB(1)
static void sitronix_ts_reset_ic(void)
{
        int ret = gpio_request(SITRONIX_RESET_GPIO1, "GPX17");//GPIO引脚申请
        s3c_gpio_cfgpin(SITRONIX_RESET_GPIO1, S3C_GPIO_OUTPUT);//
//if(ret ==0 ){
	...
	printk("reset:%s\n", __FUNCTION__);
        ...
        gpio_direction_output(SITRONIX_RESET_GPIO1, 1);
	gpio_set_value(SITRONIX_RESET_GPIO1, 1);

	msleep(1);
        gpio_direction_output(SITRONIX_RESET_GPIO1, 0);
	gpio_set_value(SITRONIX_RESET_GPIO1, 0);
